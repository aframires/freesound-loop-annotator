...
def ac_pitch_description(audiofile, fs_pool, ac_descriptors):
    ...
    # Add extra note properties for later researching on better pitch confidence
    for stat in ['dmean', 'dmean2', 'dvar', 'dvar2', 'max', 'mean', 'median', 'min', 'stdev', 'var']:
        ac_descriptors["note_pitch_{0}".format(stat)] = fs_pool['lowlevel.pitch.{0}'.format(stat)]
        ac_descriptors["note_pitch_instantaneous_confidence_{0}".format(stat)] = fs_pool['lowlevel.pitch_instantaneous_confidence.{0}'.format(stat)]
...
def analyze(audiofile, outfile=None, compute_timbral_models=False, compute_descriptors_music_pieces=False, compute_descriptors_music_samples=False, out_format="json", uri=None):
...
if __name__ == '__main__':
    ...
    Audio Commons Audio Extractor (v3-dev). Analyzes a given audio file and writes results to a JSON file.
    """)
